...
public class StudentAttendanceService
{
    ...
    public async Task<IEnumerable<StudentAttendanceRecordViewModel>> GetStudentAttendanceAsync(int studentId)
    {
        ...
        // Join AttendanceRecords -> AttendanceSessions -> Teachers
        var attendanceRecords = await _context.AttendanceRecords
            .Where(ar => ar.StudentId == studentId)
            .Join(
                _context.AttendanceSessions.Where(s => true), // All sessions (no soft delete)
                ar => ar.SessionId,
                s => s.SessionId,
                (ar, s) => new { AttendanceRecord = ar, Session = s }
            )
            .Join(
                _context.Teachers.Where(t => !t.IsDeleted), // Exclude deleted teachers
    ...
    /// Returns total sessions, present count, absent count, and attendance percentage.
    /// </summary>
    public async Task<dynamic> GetStudentAttendanceStatsAsync(int studentId)
    {
        var studentExists = await _context.Students
            .AnyAsync(s => s.StudentId == studentId && !s.IsDeleted);
